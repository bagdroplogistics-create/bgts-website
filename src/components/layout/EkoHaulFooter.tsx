    ...
    >
      <div className="container-xl">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
          ...
          <div>
            <div className="flex items-center gap-2 mb-3">
              <div className="w-7 h-7 rounded-lg bg-eko-lime/20 flex items-center justify-center">
                <Leaf size={14} className="text-eko-lime" aria-hidden="true" />
              </div>
              <span className="font-display font-black text-white text-lg">EkoHaul</span>
          ...
          {/* Quick links */}
          <nav aria-label="EkoHaul footer links">
            <ul className="flex flex-wrap gap-x-8 gap-y-2">
              {[
                { label: 'EkoHaul Plans',       href: '/ekohaul/book'     },
                { label: 'Carbon Calculator',   href: '/ekohaul/esg'      },
                { label: 'EV Fleet',             href: '/ekohaul/fleet'   },
        ...
        <div className="mt-8 pt-6 border-t border-white/10 text-white/30 text-xs">
          © {year} Baroda Goods Transport Service Pvt. Ltd. · EkoHaul is a registered brand of BGTS.
        </div>
      </div>
    </footer>
  )
